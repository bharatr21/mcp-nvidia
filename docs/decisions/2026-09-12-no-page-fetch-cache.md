...
Dedupe, it turned out, **already exists**. `deduplicate_results` (`src/mcp_nvidia/lib/deduplication.py`) drops exact-URL repeats and near-identical title-and-snippet pairs, so a client never saw that page twice. What is wrong is *when* it runs: after scoring and after the `min_relevance_score` cutoff. Duplicates are scored first, so lexical scoring counts them twice (inflating the document frequencies its term weights depend on), and under rank fusion each copy would take its own rank position and skew every rank-derived score below it.
